// Authority Evidence wire-shape builders used by the portal mock. The request
...
export function buildEvidenceAssertion(
  ...
  issuerId: AuthorityId = scenario.authority
): EvidenceAssertion {
  ...
  return {
    ...
    issuedBy: SOLMARA_AUTHORITIES[issuerId].issuer,
    providedBy: `${SOLMARA_AUTHORITIES[issuerId].origin}/`,
...
export function evidenceUrl(scenario: ScenarioResult): string {
  return `${SOLMARA_AUTHORITIES[scenario.authority].origin}/v1/evidence`;
}
