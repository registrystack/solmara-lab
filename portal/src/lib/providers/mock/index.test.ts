...
describe('MockEvidenceProvider.evaluate', () => {
  ...
  it('returns the population predicate from the central service with NIA attribution', async () => {
    ...
    expect(assertion).toMatchObject({
      ...
      issuedBy: 'urn:solmara:authority:population',
...
describe('portal authorization gates', () => {
  it('denies a cross-person request before Registry Evidence is called', async () => {
  ...
  it('denies a dependent read when the guardian link is explicitly false', async () => {
    ...
    expect(evaluation.proof.crypto.signedBy).toBe(
      'Portal authorization gate; Registry Evidence was not called'
    );
  });
...
describe('Registry Evidence wire contract', () => {
  ...
  it('separates central Evidence service identity from each legal issuer', async () => {
    const evaluation = await provider.evaluateDetailed(field('combined-support-eligibility'), ctx);
    const response = evaluation.raw.response.body as RawApplicationResponse;
    ...
    expect(
      ...
      'urn:solmara:authority:civil',
      'urn:solmara:authority:social',
      'urn:solmara:authority:social'
    ]);
    expect(
  ...
  it('uses the central Evidence endpoint and signed proof contract for child predicates', async () => {
    const evaluation = await provider.evaluateDetailed(field('date-of-birth'), ctx, {
      guardianLinkVerified: true
    });
    expect(evaluation.raw.request.url).toBe(
    ...
    expect(evaluation.proof.crypto).toMatchObject({
      signedBy: 'Civil Registration Authority through Registry Evidence',
      algorithm: 'Flattened JWS, ES256',
      issuerKey: '/.well-known/evidence/jwks.json',
      credential: 'Signed minimum-disclosure Evidence assertion'
    });
...
describe('resilience states', () => {
  ...
  it('returns the current Registry Evidence problem shape when the service is unavailable', async () => {
    const evaluation = await provider.evaluateDetailed(field('error'), ctx);
