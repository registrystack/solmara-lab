...
def main() -> int:
    ...
    for key in PINNED_IMAGE_KEYS | RUNTIME_IMAGE_KEYS:
        if key not in values:
            failures.append(f"versions.env: {key} is required")

    version = values.get("REGISTRYCTL_VERSION")
    expected_local_images = {
        "SOLMARA_EVIDENCE_IMAGE": f"solmara-lab-registry-evidence:v{version}",
        "SOLMARA_MINT_IMAGE": f"solmara-lab-registry-mint:v{version}",
    }
    for key, expected in expected_local_images.items():
        if values.get(key) != expected:
            failures.append(f"versions.env: {key} must be {expected}")

    compose_files = [ROOT / "compose.yaml", ROOT / "compose.esignet.yaml"]
    required_counts = {key: 0 for key in RUNTIME_IMAGE_KEYS}
